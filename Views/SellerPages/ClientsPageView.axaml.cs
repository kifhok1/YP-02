...
public partial class ClientsPageView : UserControl
{
    ...
    // Обработчик изменения текста в поле суммы накоплений
    // Ограничивает ввод только числами с десятичной точкой
    private void AmountOfNumber_OnTextChanged(object? sender, TextChangedEventArgs e)
    {
        AmountOfNumber.Text = LineEntryRestrictions.TextChangedPrice(AmountOfNumber.Text);
    }

    // Обработчик двойного нажатия на элемент DataGrid для редактирования клиента
    private void InputElement_OnDoubleTapped(object? sender, TappedEventArgs e)
    {
        Element.IsVisible = true; // Показ панели редактирования
        if (sender is DataGrid dataGrid)
        {
            Client selectedItem = (Client)dataGrid.SelectedItem;
            ...
            if (selectedItem != null)
            {
                ...
                AmountOfNumber.Text = selectedItem.AmountOfNumber.ToString(); // Сумма накоплений
            }
        }
    }

    // Обработчик нажатия кнопки отмены (скрытие панели и очистка полей)
    private void Button_OnClick(object? sender, RoutedEventArgs e)
    {
        ...
        AmountOfNumber.Text = "";  // Очистка поля суммы накоплений
    }
